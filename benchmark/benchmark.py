...
if __name__ == "__main__":
    ...
    parser.add_argument("--neuron-model", type=str, help="A neuron model in a local directory.")
    ...
    # Reload model neuron_config
    inference_config = InferenceConfig.from_json_file(os.path.join(args.neuron_model, "neuron_config.json"))
    model = NeuronLlamaForCausalLM(args.neuron_model)
    model.load(args.neuron_model)
    model = HuggingFaceGenerationAdapter(model)
    model_name = "results.json" if args.name is None else args.name
